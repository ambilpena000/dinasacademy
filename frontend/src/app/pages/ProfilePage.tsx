    ...
    <div className="space-y-6">
              ...
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-700 mb-4">
                    Data profil yang lengkap membantu kami memberikan rekomendasi pembelajaran yang lebih personal dan sesuai dengan tujuanmu!
                  </p>
                  <Button 
                    variant="primary" 
                    size="sm"
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
            ...
            <CardContent className="pt-6">
              <div className="text-center mb-6">
                ...
                {user.hasPurchasedPackage && user.packageType ? (
                  <Badge 
                    variant={
                      user.packageType === 'PTN Premium' ? 'blue' :
                      user.packageType === 'SKD' ? 'purple' :
                      'yellow'
                    }
                  >
                    {user.packageType}
          ...
          {/* Active Package Card - Replace Streak */}
          {user.hasPurchasedPackage && user.packageType && (
              ...
              <div className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <label className="flex items-center space-x-2 text-sm font-medium text-gray-700 mb-2">
                      ...
                      <span>Jalur Tujuan</span>
                    </label>
                    {isEditing ? (
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          type="button"
                          onClick={() => setFormData({ ...formData, targetType: 'PTN' })}
                        ...
                        >
                          <GraduationCap className={`w-5 h-5 mx-auto mb-1 ${
                            formData.targetType === 'PTN' ? 'text-[#2563EB]' : 'text-gray-600'
                          }`} />
                          <p className="font-semibold text-sm text-gray-900">PTN</p>
                        </button>
                        <button
                          type="button"
                          onClick={() => setFormData({ ...formData, targetType: 'Sekdin', targetMajor: '' })}
                          className={`p-3 rounded-xl border-2 transition-all ${
                        ...
                        >
                          <Trophy className={`w-5 h-5 mx-auto mb-1 ${
                            formData.targetType === 'Sekdin' ? 'text-[#2563EB]' : 'text-gray-600'
                          }`} />
                      ...
                      <div className="flex items-center space-x-2">
                        {formData.targetType === 'PTN' ? (
                          ...
                          <div className="px-4 py-3 bg-gray-50 rounded-xl border border-gray-200 text-gray-500">
                            Belum dipilih
                          </div>
                        )}
                      </div>
                  ...
                  {(formData.targetType || (!isEditing && user.targetUniversity)) && (
                    <>
                      <div className={formData.targetType === 'PTN' ? '' : 'md:col-span-2'}>
                        ...
                        {/* Sekdin: tidak ada prodi */}
                        {!isEditing && formData.targetType === 'Sekdin' && formData.targetMajor && (
                          <p className="sr-only" />
                        )}
                              ...
                              <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-52 overflow-y-auto">
                                {searchUniversities(univSearch || formData.targetUniversity).slice(0, 8).map(u => (
                                  <button key={u.id} type="button"
                                    ...
                                    <span className="text-gray-400 text-xs ml-1">· {u.location}</span>
                                  </button>
                                ))}
                      ...
                      {/* Prodi Tujuan — hanya tampil jika PTN */}
                      {formData.targetType === 'PTN' && (
                        <div>
                          <label className="flex items-center space-x-2 text-sm font-medium text-gray-700 mb-2">
                            <Target className="w-4 h-4" />
                            <span>Program Studi Tujuan</span>
                          </label>
                          {isEditing && formData.targetType === 'PTN' ? (
                            (() => {
                              ...
                              return programs.length > 0 ? (
                                  ...
                                  {['Saintek', 'Soshum', 'Campuran'].map(cat => {
                                    ...
                                    return filtered.length > 0 ? (
                                      <optgroup key={cat} label={cat}>
                                        {filtered.map(p => (
                                          <option key={p.id} value={p.name}>{p.name} — {p.faculty}</option>
                                        ))}
                                      </optgroup>
                                    ) : null;
                                  })}
                                ...
                                <input type="text" placeholder="Ketik program studi tujuan"
                                  value={formData.targetMajor} onChange={e => setFormData({...formData, targetMajor: e.target.value})}
                                  className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#2563EB] outline-none" />
                              );
                            })()
                          ) : (
                            <Input name="targetMajor" placeholder="Teknik Informatika"
                              value={formData.targetMajor} onChange={handleChange} disabled={!isEditing} />
                          )}
                        </div>
                      )}
                    </>
                ...
                <div>
                  ...
                  <textarea
                    name="goals"
                    rows={3}
                    placeholder="Ceritakan apa motivasi dan harapanmu..."
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          ...
          >
            ...
            <div className="p-6 space-y-6">
              ...
              {/* Kampus & Jurusan - PTN: 2 kolom sejajar, Sekdin: stacked */}
              {formData.targetType && (
                <div className={formData.targetType === 'PTN' ? 'grid grid-cols-2 gap-4' : 'space-y-4'}>
                  {/* Universitas / Sekolah Tujuan */}
                  <div>
                    <label className="flex items-center space-x-2 text-sm font-semibold text-gray-700 mb-2">
                      <GraduationCap className="w-4 h-4" />
                    ...
                    <div>
                      ...
                      {formData.targetType === 'PTN' ? (
                        (() => {
                          ...
                          const programs = selectedUni ? getPrograms(selectedUni.id) : [];
                          return programs.length > 0 ? (
                            <select value={formData.targetMajor}
                              onChange={e => setFormData({...formData, targetMajor: e.target.value})}
                              className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#2563EB] outline-none bg-white">
                              <option value="">-- Pilih Program Studi --</option>
                              {['Saintek', 'Soshum', 'Campuran'].map(cat => {
                            ...
                            <input type="text" placeholder="Ketik program studi"
                              value={formData.targetMajor} onChange={e => setFormData({...formData, targetMajor: e.target.value})}
                              className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-300 focus:ring-2 focus:ring-[#2563EB] outline-none" />
                          );
                        })()
                      ) : (
                        <Input name="targetMajor" placeholder="Contoh: Akuntansi, Manajemen"
                          value={formData.targetMajor} onChange={handleChange} />
                      )}
                    </div>
                  )}
                </div>
